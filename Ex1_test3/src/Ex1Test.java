...
class Ex1Test {
    ...
    /**
     * MY TESTS:
     * checks that the length function correctly computes the distance along the line y = x between 0 and 1 (which should be √2).
     */
    @Test
    public void testLengthSimpleLine() {
        double[] p = {0, 1};
        double x1 = 0;
        double x2 = 1;
        int segments = 100;
        double len = Ex1.length(p, x1, x2, segments);
        assertEquals(Math.sqrt(2), len, 1e-2);
    }
    /**
     * checks that PolynomFromPoints returns the correct linear polynomial passing through two given points
     */
    @Test
    public void testPolynomFromPointsLine() {
        ...
        double[] expected = {2, 2};
        assertTrue(Ex1.equals(expected, p));
    }
    /**
     * checks that sameValue finds the intersection point between y = x and y = 1 (expected x = 1)
     */
    @Test
    public void testSameValueSimple() {
        double[] p1 = {0, 1};
        double[] p2 = {1};
    ...
    /**
     * checks that the function correctly parses the string “0” into the ZERO polynomial
     */
    @Test
    public void testGetPolynomFromStringZero() {
        double[] p = Ex1.getPolynomFromString("0");
        assertTrue(Ex1.equals(p, Ex1.ZERO));
    }
    /**
     * checks that the derivative of a constant polynomial is ZERO, and the derivative of ZERO is also ZERO
     */
    @Test
    public void testDerivativeZeroAndConstant() {
        double[] c = {5};
        double[] d1 = Ex1.derivative(c);
        double[] d2 = Ex1.derivative(Ex1.ZERO);
        assertTrue(Ex1.equals(d1, Ex1.ZERO));
        assertTrue(Ex1.equals(d2, Ex1.ZERO));
    }
